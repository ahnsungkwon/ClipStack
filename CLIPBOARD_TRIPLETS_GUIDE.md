...
- `Ctrl+Alt+0`
  - 관리 창 열기
- `Ctrl+Alt+W`
  - 1~6번 슬롯의 첫 번째 항목을 보여주는 작은 위젯 열기/닫기
...
관리 창의 `비우기` 버튼을 누르면 현재 선택한 슬롯의 이름과 3개 항목이 모두 지워집니다.
...
가져온 3줄은 현재 선택된 슬롯에 바로 저장됩니다. 기존 슬롯에 내용이 있어도 새 3줄이 덮어씁니다.
...
스크립트가 실행되면 작은 `ClipStack 슬롯` 창이 같이 뜹니다.

- 1~6번 슬롯의 `위 항목` 첫 줄을 계속 보여줍니다.
- 숫자 버튼을 누르면 해당 슬롯을 바로 `Win+V`에 주입합니다.
- `새로고침`은 슬롯 표시를 다시 읽습니다.
- `관리`는 관리 창을 엽니다.
- 닫았을 때는 `Ctrl+Alt+W`로 다시 열 수 있습니다.
